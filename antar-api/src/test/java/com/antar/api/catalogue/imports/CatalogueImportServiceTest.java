 ...
 * which shadow the real docs/data placeholder-only CSVs on the test classpath.
 */
@SpringBootTest
@Transactional
class CatalogueImportServiceTest {
    ...
    void firstImportInsertsInsurersProductsAndVersionsAndSkipsUnknownParents() {
        ...
        assertThat(insurers.rowsInserted()).isEqualTo(2);
        assertThat(insurerRepository.count()).isEqualTo(2);

        // 2 valid rows imported, 1 row referencing an unknown insurer skipped, not fatal.
        assertThat(products.rowsInserted()).isEqualTo(2);
    ...
    void reImportingWithForceUpsertsWithoutCreatingDuplicates() {
        ...
        assertThat(secondRun.get(0).rowsUpdated()).isEqualTo(2);
